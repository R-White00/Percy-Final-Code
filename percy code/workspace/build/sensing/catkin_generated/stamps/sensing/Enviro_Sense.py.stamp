...
class Environment:
	...
	def batt_callback(self,vals):
		self.voltage = round(vals.y, 2)
		self.current = round(vals.x, 2)
	...
	def screen(self):
		while True:
			...
			self.draw.text((65, 45), str(self.voltage), font = self.font, fill = 1)
			self.draw.text((105, 45), 'V', font = self.font, fill = 1)
			self.draw.text((65, 30), str(self.current), font = self.font, fill = 1)
			self.draw.text((105, 30), 'A', font = self.font, fill = 1)
