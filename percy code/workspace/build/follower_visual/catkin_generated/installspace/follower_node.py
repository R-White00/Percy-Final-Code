...
lower_bgr_values = np.array([0,  60,  0])
upper_bgr_values = np.array([60, 255, 160])
lower_hsv = np.array([65, 40, 40])
...
def timer_callback(boo):
    ...
    while var < 2:
        ...
        if ((width / 2 - 110 > middle_x) or (middle_x > width / 2 + 110)):
            ...
            if (angle_rate < 3 or angle_rate > -3): #safety value for turning
                msg.angular.z = -angle_rate
            else:
                msg.angular.z = -angle_rate * 0.2
        else:
            msg.linear.x = 0.1
