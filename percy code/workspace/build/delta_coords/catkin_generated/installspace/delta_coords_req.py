...
def publish_coords(contours):
    ...
    if (cv2.contourArea(c) > 400):
        ...
        M = cv2.moments(c)        
        # calculate x pixel coordinate of center
        cX = int(M["m10"] / M["m00"])
	msg = cX	
...
def timer_callback(boo):
    ...
    while var < 2:
        ...
        img = cv2.flip(img, 0)
        cv2.imshow('REARCAMERA_img', img)
        img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        ## Gen lower mask (0-5) and upper mask (175-180) of RED
        mask1 = cv2.inRange(img_hsv, (160,30,100), (180,230,200))
        mask2 = cv2.inRange(img_hsv, (0,30,100), (10,230,200))
        full_mask = mask1 + mask2
        mask = cv2.bitwise_or(mask1, mask2 )
        #mask = cv2.inRange(img, lower_bgr_values, upper_bgr_values)
        cv2.imshow('REARCAMERA_mask', mask)
        cv2.waitKey(5)
...
if __name__ == '__main__':
    ...
    try:
        ...
        coord_pub = rospy.Publisher('delta_coord_z', Int32, queue_size = 10)
        image_sub = rospy.Subscriber('usb_cam_z/image_raw',Image, image_callback, queue_size=10)
