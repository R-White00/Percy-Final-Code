...
#define NUMSTEPPERS 1    //NOTE Change to 4 for rear robot
#define BUFFER 5
uint8_t stepDirPins[] = {30, 32, 34, 36}; //Need to add all stepper pins
uint8_t stepPins[] = {31, 33, 35, 37}; //Need to add all stepper pins
...
AccelStepper frontRightStepper = AccelStepper(motorInterfaceType, stepPins[1], stepDirPins[1]);

AccelStepper rearLeftStepper = AccelStepper(motorInterfaceType, stepPins[2], stepDirPins[2]);
...
AccelStepper Steppers[] = {frontLeftStepper, frontRightStepper, rearLeftStepper, rearRightStepper}; //for real robot
...
void wheelCommands( const std_msgs::Float64MultiArray& WheelCommand) {
  ...
  ang1 = WheelCommand.data[4]; //Front Left
  ang2 = WheelCommand.data[5]; //Front Right
  ang3 = WheelCommand.data[6]; //Rear Left
  ang4 = WheelCommand.data[7]; //Rear Right
...
void driveSteppers() {
  ...
  stepperAngles[2] = ang3 * 180 / PI;
  stepperAngles[3] = ang4 * 180 / PI;
  ...
  stepperSteps[0] = stepperAngles[0] / 1.8 * 4.5; //NOTE THESE SHOULD BE /1.8*2 TO ACCOUNT FOR GEAR RATIO
  stepperSteps[1] = stepperAngles[1] / 1.8 * 4.5;
  stepperSteps[2] = stepperAngles[2] / 1.8 * 4.5; //NOTE THESE SHOULD BE /1.8*2 TO ACCOUNT FOR GEAR RATIO
  stepperSteps[3] = stepperAngles[3] / 1.8 * 4.5;
...
void loop() {
  ...
  motorSpeed[0] = 0.088 * vel1 * (60 / (2 * PI)); //NOTE FOR TESTING TO SET THE SAME SPEED
  motorSpeed[1] = 0.088 * vel2 * (60 / (2 * PI)); //NOTE FOR TESTING TO SET THE SAME SPEED
  motorSpeed[2] = 0.088 * vel2 * (60 / (2 * PI)); //NOTE FOR TESTING TO SET THE SAME SPEED
  motorSpeed[3] = 0.088 * vel1 * (60 / (2 * PI)); //NOTE FOR TESTING TO SET THE SAME SPEED
